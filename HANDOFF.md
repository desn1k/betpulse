...
1. **Extended system health + real readiness** — implemented by the 12d follow-up PR: components for
   API readiness, ARQ queue depth/worker heartbeat, latest ingestion run, latest model evaluation,
   today's LLM token spend, and backup status (`not_configured` until Phase 14 backup lands). `/health/ready`
   now checks Postgres + Redis instead of returning a process-only stub.
