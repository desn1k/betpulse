...
async def test_system_health_reports_components(client: AsyncClient, session: AsyncSession) -> None:
    ...
    assert components["arq"]["status"] in {"ok", "degraded"}
    assert components["latest_ingestion"]["status"] == "not_configured"
    assert components["latest_model_evaluation"]["status"] == "not_configured"
    assert components["llm_spend_today"]["meta"] == {
        "tokens_in": 0,
        "tokens_out": 0,
        "tokens_total": 0,
        "cost": "0.000000",
    }
    assert components["backup"]["status"] == "not_configured"
    assert components["ops_alerts"]["status"] == "not_configured"


@pytest.mark.asyncio
async def test_system_health_reports_operational_metadata(
    client: AsyncClient, session: AsyncSession
) -> None:
    headers = await _admin_headers(session)
    now = datetime.now(UTC)
    league = League(code="META", name="Meta League")
    home = Team(name="Home", normalized_name=f"home-{uuid.uuid4().hex[:6]}")
    away = Team(name="Away", normalized_name=f"away-{uuid.uuid4().hex[:6]}")
    ...
    assert components["latest_ingestion"]["meta"]["fixtures_ingested"] == 10
    assert components["latest_model_evaluation"]["status"] == "ok"
    assert components["llm_spend_today"]["meta"] == {
        "tokens_in": 12,
        "tokens_out": 8,
        "tokens_total": 20,
